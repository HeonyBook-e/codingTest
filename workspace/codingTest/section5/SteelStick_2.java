...
public class SteelStick_2{
	public static void main(String[] args){
		SteelStick_2 getter = new SteelStick_2();
		Scanner sc = new Scanner(System.in);
		String lz = sc.next();
		
		System.out.print(getter.solution(lz));
		sc.close();
	}
	
	public int solution(String lz){
		int answer = 0;
		char[] bracket = lz.toCharArray();
		Stack<Character> stick = new Stack<>();
		
		for(int i = 0; i < bracket.length; i++) {
			if(bracket[i] == '(') stick.push('(');
			else {
				stick.pop();
				if(bracket[i-1] == '(') answer += stick.size();	//레이저인 경우
				 else answer++;			//막대기의 끝인 경우 
			}
		}
